...
public sealed class ChunkData
{
    ...
    /**
     * Client climate packets are keyed by the server's canonical chunk position. A finite-world
     * renderer may expose the same chunk through a translated client position, so retain the
     * synced data independently of ClientChunkCache's current translated window.
     */
    private static final Map<Long, ChunkData> CLIENT_CLIMATE = new ConcurrentHashMap<>();
    private static @Nullable Level clientClimateLevel;
    ...
    public static ChunkData get(LevelReader level, BlockPos pos)
    {
        ...
        {
            final ChunkData synced = CLIENT_CLIMATE.get(ChunkPos.asLong(pos));
    ...
    public static ChunkData get(LevelReader level, ChunkPos pos)
    {
        if (level instanceof Level actualLevel && actualLevel.isClientSide())
        {
            final ChunkData synced = CLIENT_CLIMATE.get(pos.toLong());
            if (synced != null) return synced;
        }
        return get(level.getChunk(pos.x, pos.z));
    }

    public static void updateClientClimate(Level level, ChunkPos pos, LerpFloatLayer rainfallLayer,
                                           LerpFloatLayer rainVarianceLayer, LerpFloatLayer baseGroundwaterLayer,
                                           LerpFloatLayer temperatureLayer, ForestType forestType)
    {
        if (clientClimateLevel != level)
        {
            CLIENT_CLIMATE.clear();
            clientClimateLevel = level;
        }
        final ChunkData data = new ChunkData(pos);
        data.onUpdatePacket(rainfallLayer, rainVarianceLayer, baseGroundwaterLayer, temperatureLayer, forestType);
        CLIENT_CLIMATE.put(pos.toLong(), data);
    }

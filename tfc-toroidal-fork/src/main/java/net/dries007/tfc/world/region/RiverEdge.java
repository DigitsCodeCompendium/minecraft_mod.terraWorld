...
public final class RiverEdge
{
    ...
    private final WorldTopology topology;
    ...
    {
        this(edge, random, WorldTopology.PLANAR);
    }

    public RiverEdge(River.Edge edge, RandomSource random, WorldTopology topology)
    {
        this.source = edge.source();
        this.drain = edge.drain();
        this.topology = topology;
        this.fractal = new MidpointFractal(random, 4, edge.source().x(), edge.source().y(), edge.drain().x(), edge.drain().y(), topology);
    ...
    {
        exactGridX = source.x() + topology.shortestGridDeltaX(exactGridX - source.x());
        exactGridZ = source.y() + topology.shortestGridDeltaZ(exactGridZ - source.y());

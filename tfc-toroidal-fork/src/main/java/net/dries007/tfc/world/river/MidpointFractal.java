...
public class MidpointFractal
{
    ...
    {
        this(random, bisections, sourceX, sourceY, drainX, drainY, WorldTopology.PLANAR);
    }

    public MidpointFractal(RandomSource random, int bisections, double sourceX, double sourceY, double drainX, double drainY, WorldTopology topology)
    {
    ...
    private double nearestX(double x)
    {
        ...
        return center + topology.shortestGridDeltaX(x - center);
    }

    private double nearestY(double y)
    {
        final double center = 0.5d * (segments[1] + segments[segments.length - 1]);
        return center + topology.shortestGridDeltaZ(y - center);
    }
}

...
public final class RegionGenerator
{
    ...
    public final WorldTopology topology;
    ...
    {
        this(settings, seed, WorldTopology.PLANAR);
    }

    public RegionGenerator(Settings settings, Seed seed, WorldTopology topology)
    {
        ...
        this.continentNoise = periodic(cellNoise.then(c -> 1 - c.f1() / (0.37f + c.f2()))
        ...
        this.rainfallVarianceNoise = periodic(new OpenSimplex2D(seed.next())
            .octaves(2)
            .spread(0.1f)
            .scaled(0f, 20f));

        this.hotSpotAgeNoise = periodic(BiomeNoise.hotSpotAge(seed.seed()).spread(128));
        this.hotSpotIntensityNoise = periodic(BiomeNoise.hotSpotIntensity(seed.seed()).spread(128));
    ...
    private Noise2D periodic(Noise2D noise)
    {
        return topology.wraps()
            ? (x, z) -> topology.samplePeriodicGrid(x, z, noise::noise)
            : noise;
    }
    ...
    {
        gridX = topology.canonicalGridX(gridX);
        gridZ = topology.canonicalGridZ(gridZ);
    ...
    public Region.Point getOrCreateRegionPoint(int gridX, int gridZ)
    {
        gridX = topology.canonicalGridX(gridX);
        gridZ = topology.canonicalGridZ(gridZ);
        return getOrCreateRegionPointUnwrapped(gridX, gridZ);
    }

    /**
     * Samples the underlying infinite regional plan without applying topology.
     * Used to blend distinct copies of the plan into a continuous torus.
     */
    public Region.Point getOrCreateRegionPointUnwrapped(int gridX, int gridZ)
    {
        return getOrCreateRegion(sampleCellUnwrapped(gridX, gridZ)).atOrThrow(gridX, gridZ);
    }
    ...
    {
        return sampleCellUnwrapped(topology.canonicalGridX(gridX), topology.canonicalGridZ(gridZ));
    }

    private Cellular2D.Cell sampleCellUnwrapped(int gridX, int gridZ)
    {

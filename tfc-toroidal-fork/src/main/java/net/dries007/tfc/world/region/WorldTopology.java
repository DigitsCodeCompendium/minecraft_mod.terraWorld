...
public interface WorldTopology
{
    ...
    record Toroidal(int minBlockX, int minBlockZ, int widthBlocks, int heightBlocks) implements WorldTopology
    {
        ...
        @Override public int climateScaleBlocks(boolean axisIsX, int requestedScale)
        {
            ...
            // Rainfall's longitudinal triangle has period 2*scale. Temperature's
            // seasonal/hemisphere triangle has period 4*scale, so its base field
            // must also use a scale that closes that longer cycle on the torus.
            final int periodMultiplier = axisIsX ? 2 : 4;

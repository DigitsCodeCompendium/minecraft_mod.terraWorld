...
public class ChunkNoiseFiller extends ChunkHeightFiller
{
    ...
    )
    {
        ...
        this.mutableDensityFunctionContext = new MutableDensityFunctionContext(new BlockPos.MutableBlockPos(), topology);
        ...
        this.interpolator = TrilinearInterpolatorList.create(settings, topology);
    ...
    public void fillFromNoise()
    {
        ...
        for (int cellX = 0; cellX < settings.cellCountXZ(); cellX++)
        {
            ...
            for (int cellZ = 0; cellZ < settings.cellCountXZ(); cellZ++)
            {
                ...
                {
                    final int columnX = chunkMinX + cellX * settings.cellWidth() + localCellX;
                    ...
                    {
                        final int columnZ = chunkMinZ + cellZ * settings.cellWidth() + localCellZ;
                        // Keep the full noise-fill path on the same coordinate setup hook used by
                        // height sampling. Toroidal compatibility folds that hook; assigning these
                        // fields directly made generated terrain ignore the fold at world seams.
                        setupColumn(columnX, columnZ);

...
public final class RegionChunkDataGenerator implements ChunkDataGenerator
{
    ...
    public ChunkData generate(ChunkData data)
    {
        ...
        if (regionGenerator.topology.wraps())
        {
            rainfallGridLayer = periodicChunkLayer(exactGridX, exactGridZ, dG, p -> p.rainfall);
            rainfallVarianceGridLayer = periodicChunkLayer(exactGridX, exactGridZ, dG, p -> p.rainfallVariance);
            temperatureGridLayer = periodicChunkLayer(exactGridX, exactGridZ, dG, p -> p.temperature);
        }

        // The base rainfall and temperature layers, scaled down to chunk resolution
        final LerpFloatLayer rainfallLayer = regionGenerator.topology.wraps() ? rainfallGridLayer : rainfallGridLayer.scaled(deltaX, deltaZ, dG);
        final LerpFloatLayer rainfallVarianceLayer = regionGenerator.topology.wraps() ? rainfallVarianceGridLayer : rainfallVarianceGridLayer.scaled(deltaX, deltaZ, dG);
        final LerpFloatLayer temperatureLayer = regionGenerator.topology.wraps() ? temperatureGridLayer : temperatureGridLayer.scaled(deltaX, deltaZ, dG);
    ...
    private LerpFloatLayer periodicChunkLayer(double x, double z, double size, ToDoubleFunction<Region.Point> value)
    {
